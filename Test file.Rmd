...
turnover <- turnover %>%
  mutate(location = case_when(personnel_subarea == "Adelaide Shift" ~ "Adelaide",
                              ...
                              personnel_subarea == "Day Elec" ~ "Sydney",
                              personnel_subarea == "Day Glass" ~ "Sydney",
...
turnover <- turnover %>%
  select(-co_cd, -company_code, -personnel_area, -personnel_subarea, -cost_ctr_code, -cost_center)
...
headcount_turnover <- combined_headcount %>%
  select(-personnel_area, -man_persno, -manager, -funct, -employee_group, -position, -cost_centre_number, -cost_centre_description, -salaried_waged, -location, -travel_cost_centre, -travel_cost_code, -full_name) %>%
  mutate(location = case_when(personnel_subarea == "Adelaide Shift" ~ "Adelaide",
                              personnel_subarea == "Adelaide Day" ~ "Adelaide",
                              personnel_subarea == "Brisbane Shift" ~ "Brisbane",
                              personnel_subarea == "Brisbane Day" ~ "Brisbane",
...
Departments & location with highest employee turnover 

Joining current headcount with new employees through the years

```{r}
headcount_turnover <- headcount_turnover%>%
  mutate(pers_no = as.integer(pers_no))
hired <- turnover %>%
  filter(turnover != -1) %>%
  select(-action_type, -reason_for_action, -turnover) %>%
  rename(c("pers_count" = "turn_abs"))
headcount_turnover <- headcount_turnover %>%
  select(-country, -employee_subgroup, -personnel_subarea)
turnover_positive <- union(headcount_turnover, hired)
turnover_negative <- turnover %>%
  filter(turnover != 1) %>%
  select(-action_type, -reason_for_action, -turnover) %>%
  distinct(pers_no, .keep_all = TRUE)
new_hired <- new_starter %>%
  select(pers_no, date, organizational_unit, personnel_subarea) %>%
  mutate(location = case_when(personnel_subarea == "Adelaide Shift" ~ "Adelaide",
                              personnel_subarea == "Adelaide Day" ~ "Adelaide",
                              personnel_subarea == "Brisbane Shift" ~ "Brisbane",
                              personnel_subarea == "Brisbane Day" ~ "Brisbane",
...
e <- as.Date(d,format="%Y-%m-%d")
new_hired$date <- e 
turnover_positive <- union(turnover_positive, new_hired)
```

Counting by location and department

```{r}
a <- turnover_positive %>% count(location, name = "positive")
b <- turnover_negative %>% count(location, name = "negative")
turnover_ratio_2 <- inner_join(a, b)

a <- turnover_positive %>% count(organizational_unit, name = "positive")
b <- turnover_negative %>% count(organizational_unit, name = "negative")
turnover_ratio_dep <- inner_join(a, b)
```

Gap between new hires and leavers

```{r}
new_starter_mov$pers_count <- 1
```


```{r}
turnover <- turnover %>% mutate(years = year(date))
```

```{r}
turnover_negative <- turnover_negative %>% mutate(years = year(date))
turnover_positive <- turnover_positive %>% mutate(years = year(date))
new_hired <- new_hired %>% mutate(years = year(date))
```
...
```{r}
new_hired_2019 <- new_hired %>%
  mutate(years = year(date)) %>%
  filter(years == 2019)
headcount_turnover <- headcount_turnover %>%
  mutate(years = year(date))
positive_2019 <- union(headcount_turnover, new_hired_2019)
turnover_negative <- turnover_negative %>%
  mutate(years = year(date))
tunrover_pos_19 <- positive_2019 %>%
  count(location, name = "positive")
tunrover_neg_19 <- turnover_negative %>%
  filter(years == 2019) %>%
  count(location, name = "negative")
turnover_19 <- inner_join(tunrover_pos_19, tunrover_neg_19)
turnover_19 <- turnover_19 %>%
  mutate(ratio = negative / positive * 100)
```

2018

```{r}
new_hired_2018 <- new_hired %>%
  mutate(years = year(date)) %>%
  filter(years == 2018)
headcount_turnover_18 <- headcount_turnover %>%
  mutate(years = year(date)) %>%
  filter(years != 2019)
positive_2018 <- union(headcount_turnover_18, new_hired_2018)
turnover_negative <- turnover_negative %>%
  mutate(years = year(date))
tunrover_pos_18 <- positive_2018 %>%
  count(location, name = "positive")
tunrover_neg_18 <- turnover_negative %>%
  filter(years == 2018) %>%
  count(location, name = "negative")
turnover_18 <- inner_join(tunrover_pos_18, tunrover_neg_18)
turnover_18 <- turnover_18 %>%
  mutate(ratio = negative / positive * 100)
```
...
```{r}
new_hired_2017 <- new_hired %>%
  mutate(years = year(date)) %>%
  filter(years == 2017)
headcount_turnover_17 <- headcount_turnover %>%
  mutate(years = year(date)) %>%
  filter(years != 2019 & years != 2018)
positive_2017 <- union(headcount_turnover_17, new_hired_2017)
turnover_negative <- turnover_negative %>%
  mutate(years = year(date))
tunrover_pos_17 <- positive_2017 %>%
  count(location, name = "positive")
tunrover_neg_17 <- turnover_negative %>%
  filter(years == 2017) %>%
  count(location, name = "negative")
turnover_17 <- inner_join(tunrover_pos_17, tunrover_neg_17)
turnover_17 <- turnover_17 %>%
  mutate(ratio = negative / positive * 100)
```

```{r}
write.csv(turnover_ratio_2,"turnover_ratio_location.csv", row.names = FALSE)
write.csv(turnover_ratio_dep,"turnover_ratio_department.csv", row.names = FALSE)
write.csv(turnover_negative,"leavers.csv", row.names = FALSE)
write.csv(new_starter_mov,"promotion_data.csv", row.names = FALSE)
write.csv(turnover,"turnover.csv", row.names = FALSE)
write.csv(new_hired,"new_employees.csv", row.names = FALSE)
write.csv(turnover_17,"turnover_17.csv", row.names = FALSE)
write.csv(turnover_18,"turnover_18.csv", row.names = FALSE)
write.csv(turnover_19,"turnover_19.csv", row.names = FALSE)
```
